...
- **Before/After** — Video is required for all code changes. For user-facing changes, show before/after with desktop and mobile, light and dark mode. For non-user-facing code/behavior changes, include a short video walking through the relevant existing functionality. Omit this section for pure non-code changes (docs, skills, config with no runtime effect).
